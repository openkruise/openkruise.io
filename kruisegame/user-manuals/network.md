...
- Value: in the format of containerName:port1/protocol1,port2/protocol2,... The protocol names must be in uppercase letters. Example: `game-server:25565/TCP`. port supports SameAsHost, which means the container uses the same port as the host. Example: `game-server:SameAsHost/UDP`. Protocol supports TCPUDP, which means TCP and UDP use the same port
