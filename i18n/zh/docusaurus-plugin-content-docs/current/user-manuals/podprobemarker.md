...
Kruise 1.8.0 版本开始，PodProbeMarker 从 alpha 提升到了 beta，且默认开启。
...
1. 通过 annotation `kruise.io/podprobe` 在 serverless Pod 上添加需要进行的探测。

2. 通过识别 serverless Pod 的 `.status.conditions[x]` 中识别到 Probe 执行结果，并执行 markerPolicy 中定义的标记操作。
