...
3. 捕获的具体范围（内存 / 文件系统）取决于运行时平台以及其背后 `SandboxTemplate` 的配置。
...
E2B SDK 在沙箱句柄上暴露了 `beta_pause()` 方法，底层调用 `POST /sandboxes/{sandboxID}/pause`。
...
with Sandbox.create(template="code-interpreter", timeout=300) as sbx:
    sbx.run_code("a = 1")
    sbx.beta_pause()  # 沙箱进入 paused 状态；sandboxID 保持不变
```
