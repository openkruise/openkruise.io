...
1. **捕获（Capture）**：sandbox-manager / Checkpoint 控制器冻结目标 Pod，将所请求的 `persistentContents`（内存和/或文件系统）写入底层 Checkpoint 存储，然后按照 `spec.keepRunning` 的设置恢复 Pod。
2. **阶段（Phase）**：Checkpoint 会依次进入 `Pending → Creating → Succeeded`（成功）或 `Failed`（失败）。只有 `Succeeded` 的 Checkpoint 才能用于克隆新沙箱。
3. **配对的 SandboxTemplate**：创建 Checkpoint 的同时，会自动生成（或复用）一个同名 `SandboxTemplate`，用于描述捕获时的 Pod 规格。约定 `Checkpoint.name == SandboxTemplate.name`。
4. **克隆 / Fork（Clone / Fork）**：以 `templateName = <checkpoint-name>` 创建新沙箱时，后端走克隆路径，从捕获状态启动一个全新的 Pod，并分配新的沙箱 ID。每次克隆都是从捕获状态独立 Fork 出的新沙箱。
5. **连接断开（Connection Drop）**：捕获期间源 Pod 被短暂暂停，所有活动 WebSocket / PTY / 命令流连接都会断开。客户端需要自行处理重连。
6. **自动回收（GC）**：`spec.ttlAfterFinished` 到期后，Checkpoint CR 及其底层资源会被自动删除。如果不配置，则由用户主动清理（`kubectl delete cp ...` 或 E2B `deleteSnapshot`）。

> 具体的 Checkpoint 语义（保留什么内容、速度、体积）取决于集群中配置的 Checkpoint 驱动。兼容性说明请参考 [E2B SDK 接入文档](./e2b-client.md)。

## Checkpoint vs. 暂停/恢复

Checkpoint 与[暂停/恢复](./sandbox-claim.md)底层共享同一套捕获机制，但面向的场景不同：
