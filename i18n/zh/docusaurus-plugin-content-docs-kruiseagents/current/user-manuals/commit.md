...
- 目标 Sandbox Pod 正在运行，并使用 commit job 支持的容器运行时。
- sandbox controller 已配置 `AGENT_JOB_IMAGE`，且该镜像包含 `commit-job` 二进制和 `nerdctl`。
- 目标节点能够访问要推送的镜像仓库。
- 如果镜像仓库使用私有 CA 或自定义 TLS 配置，目标节点的 containerd registry 配置已准备在 `/etc/containerd/certs.d` 下。
- 如果镜像仓库需要认证，同 namespace 下已创建 Docker config Secret。
...
| `spec.ttl` | `string` | 可选。`Commit` 进入 `Succeeded` 或 `Failed` 后的保留时长，例如 `24h` 或 `168h`。不配置表示不自动删除。 |
...
| `status.conditions` | `[]Condition` | commit Job 写入的详细条件信息。条件类型包括 `CommitContainer` 和 `PushCommittedImage`。 |
...
先在同 namespace 下创建 registry 认证 Secret：
