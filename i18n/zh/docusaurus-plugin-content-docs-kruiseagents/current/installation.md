...
| `image.repository`           | Controller 镜像仓库             | `openkruise-registry.cn-shanghai.cr.aliyuncs.com/openkruise/agent-sandbox-controller`                                   |
...
| `controller.repository`            | Controller 镜像仓库     | `openkruise-registry.cn-shanghai.cr.aliyuncs.com/openkruise/sandbox-manager` |
...
| Parameter                | Description  | Default                                           |
|--------------------------|--------------|---------------------------------------------------|
| `proxy.repository`       | Envoy 代理镜像仓库 | `registry-cn-hangzhou.ack.aliyuncs.com/acs/envoy` |
| `proxy.tag`              | Envoy 代理镜像版本 | `v1.4.0-gf4e7213-apsara`                          |
...
**注意：** 卸载会导致所有 Sandbox 相关的资源都被删除，包括webhook configurations, services, namespace, instances 以及所有
Sandbox 下的 Pod。 请务必谨慎操作！(不会删除CRD和namespace)
**CRD 清理**：更新和卸载时不会自动清理 CRD 资源和 Namespace，如需卸载或更新请手动操作。
