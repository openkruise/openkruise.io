...
- **Failed**: Ready condition is `False` with reason `StartContainerFailed`, `PodCreateFailed`, or `Unschedulable` — a definitive startup failure (container startup failures, image or configuration errors, Pod creation API failures, or scheduling failures such as insufficient node capacity or provider scheduling errors).
- **TimedOut**: stuck in Creating/ResourcePending longer than the pending timeout (50 seconds by default) without becoming Ready.
