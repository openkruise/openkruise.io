...
1. Install the cloud vendor's credential provisioning plugin on k8s nodes, for example: [AWS](https://github.com/awslabs/amazon-ecr-credential-helper).
2. The credentials provider the configuration of the plugin as follows:
```
apiVersion: v1
kind: ConfigMap
metadata:
...
```
3. The credentials provider requires credentials (an access key and secret access key) to sign requests to cloud provider.

<Tabs>
  <TabItem value="aws" label="aws" default>

AWS provides a variety of [authentication methods](https://docs.aws.amazon.com/sdk-for-go/v1/developer-guide/configuring-sdk.html) as follows:
    ...
    nginx:
      ...
      - completionTime: "2024-08-09T10:06:22Z"
        message: 'Failed to pull image reference "nginx:1.9.1": rpc error: code =
          ...
          dial tcp xxxxxxx:443: i/o timeout'
        phase: Failed
        startTime: "2024-08-09T10:03:52Z"
        tag: 1.9.1
  pulling: 0
