...
def link(e_path, z_path, strict=True):
    ...
    en_counter = Counter(en_links)
    zn_counter = Counter(zn_links)
        
    if dict(en_counter) != dict(zn_counter):
        diff_en = list((en_counter - zn_counter).elements())
        diff_zn = list((zn_counter - en_counter).elements())
        diff_msg = f"\n  Only in EN: {diff_en}\n  Only in ZH: {diff_zn}"
        msg = (f"Inconsistent link count: {e_path} ({len(en_links)}) vs {z_path} ({len(zn_links)}){diff_msg}")
        if strict:
            log(msg)
        else:
            warn(msg)
...
def highlight(e_path, z_path, strict=True):
    ...
    en_counter = Counter(en_emphasizes)
    cn_counter = Counter(cn_emphasizes)
        
    if len(cn_emphasizes) != len(en_emphasizes):
        diff_en = list((en_counter - cn_counter).elements())
        diff_cn = list((cn_counter - en_counter).elements())
        diff_msg = f"\n  Only in EN: {diff_en}\n  Only in ZH: {diff_cn}"
        msg = (f"Inconsistent highlight count: {e_path} ({len(en_emphasizes)}) vs {z_path} ({len(cn_emphasizes)}){diff_msg}")
        if strict:
            log(msg)
        else:
            warn(msg)
...
def title_count(e_path, z_path, strict=True):
    ...
    en_counter = Counter(en_titles)
    zn_counter = Counter(zn_titles)
    
    if len(en_titles) != len(zn_titles):
        diff_en = list((en_counter - zn_counter).elements())
        diff_zn = list((zn_counter - en_counter).elements())
        diff_msg = f"\n  Only in EN: {diff_en}\n  Only in ZH: {diff_zn}"
        msg = f"Inconsistent title count: {e_path} ({len(en_titles)}) vs {z_path} ({len(zn_titles)}){diff_msg}"
        if strict:
            log(msg)
        else:
            warn(msg)
...
def inline_count(e_path, z_path, strict=True):
    ...
    en_counter = Counter(en_ins)
    zn_counter = Counter(zn_ins)
    
    if len(en_ins) != len(zn_ins):
        diff_en = list((en_counter - zn_counter).elements())
        diff_zn = list((zn_counter - en_counter).elements())
        diff_msg = f"\n  Only in EN: {diff_en}\n  Only in ZH: {diff_zn}"
        msg = f"Inconsistent inline code count: {e_path} ({len(en_ins)}) vs {z_path} ({len(zn_ins)}){diff_msg}"
        if strict:
            log(msg)
        else:
            warn(msg)
...
advisory_handlers = [title_count, link, highlight, inline_count, lexical_analysis]
...
if __name__ == '__main__':
    ...
    # Log new words for manual review (do NOT auto-write to pre_dict.json)
    if pre_dict_increment:
        print(f"\nNew words detected (not in pre_dict.json): {sorted(pre_dict_increment)}")
        print("If these are valid terms, manually add them to pre_dict.json.")
