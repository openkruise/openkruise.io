...
- Faster image pulling, because we can reuse most of image layers pulled by the old image and only to pull several new layers.
...
Containers in the same Pod might have dependencies, which means the application in one container runs depending on another container. For example:
